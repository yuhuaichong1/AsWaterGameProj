...
{
    /// <summary>生成长桌目标颜色，保证初始开放目标至少包含锁瓶外可消颜色。</summary>
    public static class LevelPocketColorPlanner
    {
        const int InitialOpenPocketCount = 2;

        public static void BuildPocketColors(
            IList<CupData> cups, int levelIndex, List<int> output, bool useUnityRandom)
        {
            output.Clear();
            if (cups == null)
                return;
            ...
            foreach (var cup in cups)
            {
                ...
                {
                    if (c <= 0) continue;
                    colorCount.TryGetValue(c, out var n);
                    n++;
                    if (n % 4 == 0)
                    {
                        output.Add(c);
        ...
        public static List<int> GetOutsideCompletableColors(IList<CupData> cups)
        {
            ...
            if (cups != null)
            {
                foreach (var cup in cups)
                {
                    ...
                    {
                        if (color <= 0) continue;
                        outsideCounts.TryGetValue(color, out var n);
                        outsideCounts[color] = n + 1;
                    }
                }
            }
        ...
        static void EnsureInitialOpenPocketHasOutsideCompletableColor(IList<CupData> cups, List<int> pocketColors)
        {
            ...
            var preferred = BuildPreferredInitialColors(cups, outsideCompletable);
            var openCount = Mathf.Min(InitialOpenPocketCount, pocketColors.Count);
            for (var i = 0; i < openCount; i++)
            {
                if (preferred.Contains(pocketColors[i]))
                    return;
            }
        ...
        static List<int> BuildPreferredInitialColors(IList<CupData> cups, List<int> outsideCompletable)
        {
